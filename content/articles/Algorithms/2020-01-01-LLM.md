...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        scores = scores / torch.sqrt(torch.tensor(self.d_head)) # (batch_size, n_head, seq_len, seq_len)
        attn_weights = torch.nn.functional.F.softmax(scores, dim=-1)

        # 4.计算输出
        output = torch.matmul(attn_weihts, V) # (batch_size, n_head, seq_len, d_head)

        # 5.合并多头
        output = output.transpose(1, 2).view(batch_size, seq_len, self.d_model) # (batch_size, seq_len, d_model)
